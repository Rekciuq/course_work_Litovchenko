...
namespace ЭС
{
    public partial class frmMain : Form
    {
        ...
        {
            this.Text = "Экспертна система - " + tempFile.Substring(tempFile.LastIndexOf('\\') + 1);
        ...
        private void SaveES(string file)
        {
            try
            {
                ...
                fs.Close();
                MessageBox.Show("Экспертна система успішно збережена!", "Збереження", MessageBoxButtons.OK, MessageBoxIcon.Information);
            }
            catch (Exception ex)
            {
                MessageBox.Show(ex.Message, "Помилка", MessageBoxButtons.OK, MessageBoxIcon.Error);
        ...
        public void StartConsult()
        {
            try
            {
                if (tempES.Goal != null)
                {
                    ...
                    if (f.Truly == Rightly.Unknown)
                        MessageBox.Show("Не вийшло встановити правду!");
                    else
            ...
            {
                MessageBox.Show("Не вийшло зробити вивід! Причина: " + de.Message, "Помилка", MessageBoxButtons.OK, MessageBoxIcon.Error);
            }
            catch (Exception ex)
            {
                MessageBox.Show("Незрозуміла помилка!", "Помилка", MessageBoxButtons.OK, MessageBoxIcon.Error);
            }
        }
        ...
        private void frmMain_FormClosing(object sender, FormClosingEventArgs e)
        {
            if (!saved)
            {
                DialogResult dr = MessageBox.Show("Зберегти перед завершенням роботи?", "Вихід", MessageBoxButtons.YesNoCancel, MessageBoxIcon.Question);
                switch (dr)
                {
